...
```bash
url-summarizer$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
url-summarizer$ sam local start-api
url-summarizer$ curl http://localhost:3000/
```
...
```bash
url-summarizer$ sam logs -n HelloWorldFunction --stack-name url-summarizer --tail
```
...
```bash
sam delete --stack-name url-summarizer
```
